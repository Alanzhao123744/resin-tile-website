...
var limits={asa:{'20ft':22000,'40ft':28000,'40hc':30000},pvc:{'20ft':18000,'40ft':26000,'40hc':28000}};
...
function calc(){var d=wgt[p.value][t.value],lim=limits[p.value][c.value],sqm=Math.floor(lim/d),pal=Math.round(sqm/pp[p.value]);document.getElementById('calcSqm').textContent='~'+sqm.toString().replace(/\B(?=(\d{3})+(?!\d))/g,'.');document.getElementById('calcDetail').textContent='~'+pal+' pallets · '+d+' kg/sqm · max '+lim.toString().replace(/\B(?=(\d{3})+(?!\d))/g,'.')+' kg load';}
