...
function calc(){var d=wgt[p.value][t.value],lim=limits[p.value][c.value],sqm=Math.floor(lim/d),pal=Math.round(sqm/pp[p.value]);document.getElementById('calcSqm').textContent='~'+sqm.toLocaleString('en-US');document.getElementById('calcDetail').textContent='~'+pal+' pallets · '+d+' kg/sqm · max '+lim.toLocaleString('en-US')+' kg load';}
